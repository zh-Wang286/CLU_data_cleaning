...
`CLUProcessor` 采用模块化设计，将分析流程分解为以下核心方法：

### 🏗️ **核心方法映射**
| 重构前方法名 | 重构后方法名 | 功能描述 |
|------------|------------|----------|
| `detect_intra_intent_outliers()` | `find_outliers_within_intents()` | 意图内异常点检测 |
| `detect_boundary_violations()` | `analyze_boundary_violations()` | 全局边界违规分析 |
| `detect_targeted_boundary_violations()` | `analyze_targeted_boundary_violations()` | **新增**局部边界违规分析 |
| `audit_global_clusters()` | `audit_clusters_globally()` | 全局聚类审计 |
| `enrich_low_utterance_intents()` | `generate_utterance_candidates()` | 语料增广生成 |
...
### 3.2. 意图内异常点检测 (`find_outliers_within_intents()`)
...
### 3.3. 全局聚类审计 (`audit_clusters_globally()`)
...
#### 3.4.1. 全局边界分析 (`analyze_boundary_violations()`)


聚类审计提供了一个宏观的视图，而这个方法则提供了一种更精细、更具统计意义的方式来审查意图边界。它会逐一检查**所有语料**，并计算它在统计上有多大概率可以被归属到另一个意图。


#### 3.4.2. 局部边界分析 (`analyze_targeted_boundary_violations()`) **[新增]**


这是重构后新增的核心方法，专门用于深入分析**指定意图子集**之间的边界混淆。与全局分析不同，它具有以下特点：

- **聚焦分析**: 只分析用户指定的意图，排除无关意图的干扰
- **全局一致性**: 使用与全局分析相同的PCA空间，确保结果可比性
- **高精度**: 针对特定意图对构建更精确的统计模型
...
两种边界分析方法都采用相同的核心算法流程：

1. **处理"维度灾难" (PCA)**: 使用主成分分析降维到稳定的低维空间
2. **构建概率分布**: 计算每个意图的均值向量和协方差矩阵  
3. **衡量归属可能性 (马氏距离)**: 计算语料到其他意图分布的马氏距离
4. **转换为概率 (p-value)**: 通过卡方分布转换为统计显著性指标

### 3.5. 数据增广 (`generate_utterance_candidates()`)
